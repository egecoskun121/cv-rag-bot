 ...
 * simply doesn't emit events (see {@code RagService}). Uses the schema-registry-aware
 * template from {@link QaEventSchemaConfig} — {@code @Qualifier} disambiguates it
 * from Spring Boot's own auto-configured (plain JSON) {@code KafkaTemplate}.
 */
@Component
...
public class QaEventPublisher {
    ...
    public QaEventPublisher(@Qualifier("qaEventKafkaTemplate") KafkaTemplate<String, QaEvent> kafkaTemplate,
